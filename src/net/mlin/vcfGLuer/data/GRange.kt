 ...
 */
data class GRange(val rid: Short, val beg: Int, val end: Int) :
    Comparable<GRange>, java.io.Serializable {
    init {
        require(rid >= 0)
        require(beg > 0)
        require(end >= beg)
    ...
    fun contains(other: GRange): Boolean =
        (rid == other.rid && beg <= other.beg && end >= other.end)
